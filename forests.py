...
def make_sub(prediction, name = "my"):
	# Needs to count the number of columns prediction has.

	ids = np.array(prediction.iloc[:,0])

	first = np.array(prediction.iloc[:,1])
	second = np.array(prediction.iloc[:,2])
	third = np.array(prediction.iloc[:,3])
	fourth = np.array(prediction.iloc[:,4])
	fifth = np.array(prediction.iloc[:,5])

	combined = np.concatenate((first,second,third,fourth,fifth))
	combined_ids = np.concatenate((ids,ids,ids,ids,ids))

	sub = pd.DataFrame([combined_ids, combined.T], index=["id", "country"]).T

	subname = name + "_submission.csv"
	sub.to_csv(subname, index=False, quoting=csv.QUOTE_NONE)
	return sub

def items_in_top_k(pred, target):
	if target.dtypes.ix["country_destination"] == "int64":
		target = target.country_destination.map(dicts.country)

	prediction = np.array(pred)[:, 1:]
	tar = np.array(target)
	tar = tar.reshape((len(tar),1))
	matrix = prediction == tar
	out = np.sum(matrix, axis=0)/len(matrix)
	out = out.cumsum()

	NDCG = calc_NDCG(prediction, tar)
